...
2. En tu máquina está la carpeta **`cvea-articulos-repo`** (en la raíz del proyecto) con los dos HTML y un README; esa carpeta no se sube al repo de la web (está en `.gitignore`). Úsala para el nuevo repo:
   ...
   - `git init`
   - `git add .`
   - `git commit -m "Artículos Resources HTML"`
...
4. La web del hub ya tiene los enlaces apuntando a `https://angelucv.github.io/cvea-articulos/...`; en cuanto el repo exista y Pages esté activo, se verán los artículos en el navegador.
